...
public:
    ...
    static void init_timer(double est_clock_freq)
    {
    ...
    static double sync_tsc_time()// - 130000ns on TC's EC2! Local performance = 50ns
    {
    ...
    static ALWAYS_INLINE double get_time() //a.k.a. getTODTime()
    {
     ...
     */
    static ALWAYS_INLINE double get_tsc_time()//Takes 9.4ns on TC's EC2! Local performance = 7.4ns.
    {
    ...
    /*!Return the number of seconds since initTimer. Based on TSC !
     @Remark The TSC is incremented by the base multiplier once every reference clock. In other words, the same value could be returned multiple times!
     */
    static ALWAYS_INLINE double get_tsc_time_p(int &chip, int &core)//Takes ?.?ms on TC's EC2! Local performance = 16ns.
    {
    ...
    /*!Return the number of clock ticks since some past event - a modern constant_tsc and nonstop_tsc CPU is assumed.
     @Remark The TSC is incremented by the base multiplier once every reference clock. In other words, the same value could be returned multiple times!
     */
    static ALWAYS_INLINE uint64_t get_tsc_tick()
    {
        //#ifdef __x86_64
    ...
    static ALWAYS_INLINE uint64_t get_tsc_tick_p(int &chip, int &core)
    {
        uint64_t timelo, timehi;
        uint32_t chx;
...
private:
    ...
    static ALWAYS_INLINE double get_tod_seconds()
    {
        struct timeval tv;

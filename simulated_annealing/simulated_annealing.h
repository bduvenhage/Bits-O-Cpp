...
//! Based on Schraudolph 1999 - Valid for x in approx range (-700, 700).
ALWAYS_INLINE double fast_exp(const double x)
{
    union{double d_; int32_t i_[2];} uid;
    uid.i_[0] = 0;
    uid.i_[1] = int32_t(double((1<<20) / log(2.0)) * x + double(1023 * (1<<20) - 0));
    return uid.d_;
}
